...
try:
    ...
    for j in range(2,4):
        ...
        for i in range(1,11):
            ...
            # 수강후기 갯수 구하기
            # 1. 특정 글의 첫 수강후기 css selector
            # #reviewNumStart_select = f"#tbodyEpilogue > dd:nth-child(3) > p"
            
            # 첫 수강후기는 nth-child(3)으로 시작
            
            # 2. 특정 글의 마지막 수강후기 css selector
            # #reviewNumEnd_select = f"#tbodyEpilogue > dd:nth-child(12) > p"
            
            # 3. 특정 글의 마지막 수강후기 css selector
            # #reviewNumNon_select = f"#tbodyEpilogue > dd > p"            
...
print(len(name_list), len(title_list), len(score_list), len(period_list_1), len(period_list_2), len(local_list), len(review_list), len(tType_list), len(reviewNum_list))

# 결과값 데이터 프레임 정의
df = pd.DataFrame({'title_list':title_list, 'tType_list':tType_list, 'local_list':local_list, 'name_list':name_list, 'score_list':score_list, 'period_list_1':period_list_1, 'period_list_2':period_list_2, 'review_list':review_list, 'reviewNum_list':reviewNum_list})
