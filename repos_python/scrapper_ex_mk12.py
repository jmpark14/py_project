    ...
    for j in range(2,3):
        ...
        for i in range(1,11):
            ...
            for k in range (reviewNum):
                ...
                trainNum_option = period_list[1].replace(")","")
                ...
                trainNum_click = f"#srchTracseTme > option"
                trainNum_option_val = driver.find_element(By.CSS_SELECTOR, trainNum_click)
                if trainNum_option_val.text == trainNum_option:
                    # 수강정보의 회차와 옵션의 회차가 같은 경우 selected 속성 추가
                    trainNum_option_val.setAttribute('selected')                
